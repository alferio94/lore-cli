...
The root TUI offers `Status`, `Login`, `Logout`, `Doctor`, `Install`, `Update`, and `Quit`. `Install` is Pi-first: Pi is selectable and recommended, Codex is supported with managed remote MCP + skills projection, Antigravity is the supported prompt-and-skills MVP target, and Claude Code stays visible as a roadmap-only `Coming soon` target. OpenCode is no longer a supported install target; the prior bounded config-only surface under `~/.config/opencode` has been removed. `Update` can surface binary update availability in the background and asks for confirmation before running the binary-only CLI updater.
