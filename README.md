...
The root TUI offers `Status`, `Login`, `Logout`, `Doctor`, `Install`, `Update`, and `Quit`. `Install` is Pi-first: Pi is selectable and recommended, Codex is supported with managed remote MCP + skills projection, Antigravity is the supported prompt-and-skills MVP target, OpenCode is supported again as a bounded config-only projection (AGENTS.md, `~/.config/opencode/skills/<phase>/SKILL.md`, a Lore-owned `opencode.json` block, and `lore-install.json`) with optional remote MCP and the bounded `opencode-plugins` default bundle — `background-agents.ts` and `model-variants.ts` are copied to `~/.config/opencode/plugins/` and the community `opencode-subagent-statusline` is the only plugin referenced from `tui.json` (the local plugin .ts files are NOT registered in `tui.json`) — with explicit `sdd-engram` / `logo` exclusions, and Claude Code stays visible as a roadmap-only `Coming soon` target. The OpenCode install enforces a fail-closed `mcp.lore` ownership check: a non-Lore-owned `mcp.lore` block (missing the `managed_by: lore-cli` marker or owned by another tool) is rejected with a typed conflict error and a backup of the existing `opencode.json` rather than silently overwritten. `Update` can surface binary update availability in the background and asks for confirmation before running the binary-only CLI updater.
