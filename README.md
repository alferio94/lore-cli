...
The root TUI offers `Status`, `Login`, `Logout`, `Doctor`, `Install`, `Update`, and `Quit`. `Install` is Pi-first: Pi is selectable and recommended, Codex is supported with managed remote MCP + skills projection, Antigravity is the supported prompt-and-skills MVP target, OpenCode is supported again as a bounded config-only projection (AGENTS.md, `~/.config/opencode/skills/<phase>/SKILL.md`, a native `opencode.json` with `$schema: https://opencode.ai/config.json`, a native `agent` overlay wiring each SDD phase to its `SKILL.md` via `{file:...}` references, a native `skills` block, and `lore-install.json`) with optional remote MCP and the bounded `opencode-plugins` default bundle — `background-agents.ts` and `lore-models.ts` are copied to `~/.config/opencode/plugins/` and the community `opencode-subagent-statusline` is the only plugin referenced from `tui.json` (the local plugin .ts files are NOT registered in `tui.json`'s singular `plugin` string array) — with explicit `sdd-engram` / `logo` exclusions. The managed `lore-models.ts` plugin keeps the provider/model/variant discovery cache behavior of the previous `model-variants.ts` asset and additionally exposes a safe `opencode.json` hot-edit entrypoint for in-OpenCode model/variant configuration of Lore-managed agents (including `lore-worker` and the SDD phase agents), and Claude Code stays visible as a roadmap-only `Coming soon` target. The native `opencode.json` carries NO top-level Lore-only `lore` metadata block; the `mcp.lore` remote MCP entry is the only Lore-owned top-level subtree under `mcp` and is written only when `lore-server-mcp` is selected. The OpenCode install enforces a fail-closed `mcp.lore` ownership check: a non-Lore-owned `mcp.lore` block (missing the `managed_by: lore-cli` marker or owned by another tool) is rejected with a typed conflict error and a backup of the existing `opencode.json` rather than silently overwritten. Legacy installs that shipped the previous top-level `lore` block in `opencode.json`, or the plural `plugins` array of objects in `tui.json`, are silently repaired to the native shape on the next run; user-owned top-level keys (e.g. `theme`, custom `mcp.<other>` entries, custom `agent.<other>` overrides) are preserved. `Update` can surface binary update availability in the background and asks for confirmation before running the binary-only CLI updater.
...
`install` reuses healthy saved Lore login state, runs the same config `/healthz` `/readyz` `/v1/me` preflight as `status`, and keeps Pi as the default recommended target. The current Pi slice uses hosted Lore MCP via `pi-mcp-adapter` as the default backend: it installs the portable Lore agent pack into the managed `~/.pi/agent` Pi runtime files, manages `settings.json` with the pinned hosted MCP package and `mcp.json` with HTTP MCP configuration pointing to the hosted Lore MCP endpoint (`<lore_url>/v1/mcp`) with a bearer token materialized in plaintext on disk (matching the Antigravity local plaintext-token tradeoff), and preserves existing package order/other entries while keeping `lore-install.json` as bookkeeping-only metadata rather than a runtime source. The default install does NOT include any deprecated `extensions/lore-memory.ts` asset — the deprecated Pi-native memory extension was removed from the install bundle and is not available in any install path. The optional `extensions/lore-footer.ts` (UI status) is available for explicit opt-in via `lore install --target pi --component pi-extensions`, but is not installed by default. A default complete install also renders an extended-skills bundle (skill-creator, skill-registry, judgment-day) into `skills/<name>/SKILL.md`; these are installer-managed CLI convenience skills separate from core-pack agents, and `lore install` reruns reconcile the bundle while `lore update` does not touch skill files or runtime content. It also renders managed global agent overlays under `~/.pi/agent/agents/lore-managed-*.md`; runtime resolution is `builtin < managed < user`; user-owned collisions are reported and left untouched; and stale managed overlays are backed up before delete/update so rollback can restore them from the managed backup root. Lore-managed installs ignore project `.pi/agents` by default at runtime; opt in explicitly with `settings.json.lore.agent_resolution.project_agents=enabled` when you really want project-local agents back in play. When a legacy managed `extensions/lore-delegation.ts` exists from an older install, `lore install` reports a scoped cleanup action, backs that file up under the managed backup root, and deletes only that obsolete path during apply. OpenCode is supported again as a bounded config-only projection under `~/.config/opencode/`: managed `AGENTS.md` and `skills/<phase>/SKILL.md` (rendered from the portable agentpack with the canonical SDD phase naming), a NATIVE `opencode.json` shape with `$schema: https://opencode.ai/config.json`, `theme: system`, the native `agent` overlay wiring every canonical SDD phase (`sdd-init`, `sdd-explore`, `sdd-propose`, `sdd-spec`, `sdd-design`, `sdd-tasks`, `sdd-apply`, `sdd-verify`, `sdd-archive`) to its `~/.config/opencode/skills/<name>/SKILL.md` prompt via `{file:./skills/<name>/SKILL.md}` references, and a native `skills` block declaring `path: ~/.config/opencode/skills`. The native `opencode.json` carries NO top-level Lore-only `lore` metadata block. When `lore-server-mcp` is selected, the installer writes a documented top-level `mcp.lore` remote entry — `type: remote`, normalized `<lore_url>/v1/mcp` URL, `headers.Authorization = Bearer <saved-token>`, and a `managed_by: lore-cli` ownership marker — and the bearer token is persisted in plaintext (the same explicit local plaintext-token tradeoff used for Antigravity and Codex). The bounded `opencode-plugins` default bundle lives under `~/.config/opencode/plugins/` (`background-agents.ts` and `lore-models.ts` are copied to `plugins/`; only the community `opencode-subagent-statusline` is registered in `~/.config/opencode/tui.json` via the native singular `plugin` string array — the local plugin .ts files are NOT in `tui.json`). The managed `lore-models.ts` plugin keeps the provider/model/variant discovery cache behavior of the previous `model-variants.ts` asset and additionally exposes a safe `opencode.json` hot-edit entrypoint for in-OpenCode model/variant configuration of Lore-managed agents (the primary `lore` orchestrator, the `lore-worker` repository worker, and every canonical SDD phase agent); the in-OpenCode configuration is rendered as `agent.<name>.model` and `agent.<name>.variant` and is preserved across reinstall. `tui.json` uses `$schema: https://opencode.ai/tui.json`, `theme: system`, and that singular `plugin` string array. The manifest `lore-install.json` carries backup-before-overwrite semantics. The current OpenCode slice is config-only: no profiles, bootstrap, or runtime subagents. The explicit `sdd-engram` and `logo` plugins are never bundled, rendered, or registered by the installer — the exclusion list is enforced at the embed.FS static guard, the plugin asset reader, and the `tui.json` plugin allowlist. The installer enforces a fail-closed `mcp.lore` ownership check: the rendered `mcp.lore` block carries a `managed_by: lore-cli` marker, and a non-Lore-owned `mcp.lore` block (missing the marker or owned by another tool) is rejected with a typed conflict error and a backup of the existing `opencode.json` at the managed backup root, rather than silently overwritten. Legacy installs that shipped the previous top-level `lore` block in `opencode.json`, or the plural `plugins` array of objects plus a top-level `lore` block in `tui.json`, are silently repaired to the native shape on the next run; user-owned top-level keys (e.g. `theme`, custom `mcp.<other>` entries, custom `agent.<other>` overrides) are preserved during the migration. When `lore-server-mcp` is selected, the saved token is persisted in plaintext under `mcp.lore.headers.Authorization`; the install summary surfaces a `plaintext-token warning` that names the path, the server URL, and the auth header name, and never embeds the saved token. The supported install target matrix is Pi, OpenCode, Codex, and Antigravity. Antigravity is the supported prompt-and-skills MVP target and includes the same extended-skills bundle; the contract is prompt append/merge plus managed skills, and `lore-cli`/`internal/agentpack` is the source of truth for the managed persona, config, and skills that Lore installs. `lore install` writes the managed Gemini agent profile to `~/.gemini/config/agents/lore.json`; its English `description`, `default: true`, and generated `systemInstruction` come from the portable agentpack plus a small Antigravity/Gemini runtime suffix rather than from a separate hand-maintained prompt asset. The Gemini profile intentionally omits a fixed `tools` field because Lore MCP tools are exposed by server role and permissions. Optional Antigravity MCP writes direct remote config to `~/.gemini/config/mcp_config.json` with `mcpServers.lore.serverUrl = ${lore_url}/v1/mcp` and `headers.Authorization = Bearer <saved-token>`. That header is plaintext on disk because current Gemini/Antigravity compatibility requires it, so Lore keeps the tradeoff explicit and limits it to the local Gemini MCP config instead of Pi-managed files. Pi-style overlay emulation is out of scope, and no auto-install, daemon, or autostart guarantee is claimed. Codex writes Lore-managed `~/.codex/agents.md`, `~/.codex/config.toml`, skills, and manifest state with backup-before-overwrite semantics; the managed Codex MCP entry points to `${lore_url}/v1/mcp` and writes a static `Authorization = "Bearer <saved-token>"` header under `[mcp_servers.lore.http_headers]`, matching the same explicit local plaintext-token tradeoff used for direct remote MCP targets. Codex still does not install Codex itself, run `codex exec`, or claim live subagent execution. Claude Code remains visible as a `Coming soon` roadmap target.
