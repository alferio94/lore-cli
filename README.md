...
The root TUI offers `Status`, `Login`, `Logout`, `Doctor`, `Install`, `Update`, and `Quit`. `Install` is Pi-first: Pi is selectable and recommended, Antigravity is the supported prompt-and-skills MVP target, and Claude Code, OpenCode, and Codex stay visible as roadmap-only `Coming soon` targets. `Update` can surface binary update availability in the background and asks for confirmation before running the binary-only CLI updater.
...
- Lore CLI no longer ships a local MCP bridge or auth-file flow: there is no `lore mcp serve`, `lore mcp proxy`, `lore mcp doctor`, hidden `lore api mcp-call`, or `lore_mcp_auth.json` path in this repository slice.
