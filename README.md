...
The root TUI offers `Status`, `Login`, `Logout`, `Doctor`, `Install`, `Update`, and `Quit` in a centered single-column wizard: there is no sidebar, selected rows keep their one-line help next to the label, long detail bodies scroll with arrows/`j`/`k`/PgUp/PgDn/`g`/`G`, and narrow/short terminals suppress secondary copy before hiding active prompts or keyboard hints. `Install` uses the same terse picker with Pi recommended by default, OpenCode/Codex/Antigravity supported, Claude Code visible as a roadmap-only `Coming soon` target, optional `? details` for long technical target notes, and a short confirmation summary before execution. The underlying install contracts are unchanged: OpenCode remains a bounded config-only projection with managed `AGENTS.md`, agentpack-rendered native prompt files under `~/.config/opencode/prompts/`, native `opencode.json` agents (`default_agent=lore`, `mode: "primary"` for `lore`, `mode: "subagent"` for `lore-worker` and SDD agents), default `mcp.lore` plus `mcp.context7`, `lore-install.json`, and a native-safe `tui.json` with an empty native `plugin` array. Legacy runtime/statusline plugins (`background-agents.ts`, `lore-models.ts`, `model-variants.ts`, and `opencode-subagent-statusline.ts`) are not installed; prior Lore-managed copies are cleaned up only when manifest ownership proves they are safe to remove. The native `opencode.json` carries NO top-level Lore-only `lore` metadata block, and the OpenCode install enforces fail-closed `mcp.lore` and `mcp.context7` ownership checks so foreign MCP blocks are rejected with backup-path guidance rather than silently overwritten. Legacy installs that shipped the previous top-level `lore` block in `opencode.json`, or the plural `plugins` array of objects in `tui.json`, are silently repaired to the native shape on the next run; user-owned top-level keys (e.g. `theme`, custom `mcp.<other>` entries, custom `agent.<other>` overrides) are preserved. `Update` can surface binary update availability in the background and asks for confirmation before running the binary-only CLI updater.
...
- Antigravity install writes the managed Gemini agent profile to `~/.gemini/config/agents/lore.json` and writes `~/.gemini/config/mcp_config.json` with `mcpServers.lore.serverUrl = <lore-url>/v1/mcp` plus `headers.Authorization = Bearer <saved-token>` and `mcpServers.context7.serverUrl = https://mcp.context7.com/mcp`; rerun `lore install` after login or server changes.
- Codex install writes `~/.codex/config.toml` with `[features].multi_agent = true`, `[mcp_servers.lore] url = <lore-url>/v1/mcp`, `[mcp_servers.context7] url = https://mcp.context7.com/mcp`, `[mcp_servers.lore.http_headers] Authorization = Bearer <saved-token>`, and explicit Lore MCP per-tool approvals only for `lore_me`, `lore_project_*`, and `lore_memory_*`; it also writes `sdd-strong.config.toml`, `sdd-mid.config.toml`, and `sdd-cheap.config.toml`. Rerun `lore install` after login or server changes. Hooks and MCP handshake/transport changes are not installed.
...
Context7 opt-in/out flags are intentionally not part of this change; supported targets receive the managed no-auth Context7 remote by default alongside Lore MCP.
