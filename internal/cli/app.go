...
func (a *App) runDoctor(actions InteractiveActions, args []string) int {
	...
	fs.Usage = func() {
		...
		fmt.Fprintln(a.Stderr, "Run actionable config, network, readiness, auth, Pi, and OpenCode native-agent diagnostics.")
	}
	if err := fs.Parse(args); err != nil {
		return 1
	}
...
func (a *App) runInstall(_ InteractiveActions, args []string) int {
	...
	fs.Usage = func() {
		...
		fmt.Fprintln(a.Stderr, "Codex grants only explicit Lore MCP per-tool approvals for lore_me, lore_project_*, and lore_memory_* in the managed Lore MCP block; hooks.json and MCP handshake/transport changes are out of scope.")
		...
		fmt.Fprintln(a.Stderr, "Codex install writes ~/.codex/config.toml with enforced [features].multi_agent=true, a managed Lore remote MCP entry pointing at /v1/mcp, a plaintext Authorization bearer token under [mcp_servers.lore.http_headers], and explicit Lore MCP per-tool approvals; SDD lane profiles are rendered separately.")
