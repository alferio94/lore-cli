...
func (a *App) runInstall(_ InteractiveActions, args []string) int {
	...
	fs.Usage = func() {
		...
		fmt.Fprintln(a.Stderr, "OpenCode install writes ~/.config/opencode/AGENTS.md, skills/<phase>/SKILL.md, opencode.json, lore-install.json, the bounded opencode-plugins bundle (background-agents.ts, lore-models.ts, and the community opencode-subagent-statusline referenced from tui.json), and a default mcp.lore remote MCP entry. The opencode-plugins and lore-server-mcp components are on by default for OpenCode; explicit exclusions (sdd-engram, logo) are never bundled or registered. The saved token is persisted in plaintext under mcp.lore.headers.Authorization and the install summary surfaces a plaintext-token warning that never embeds the saved token. OpenCode also gets default_agent=lore plus a `mode: \"primary\"` orchestrator, a `lore-worker` repository worker subagent, and `mode: \"subagent\"` on every non-lore Lore-managed agent. User-chosen `model` and `variant` values on any Lore-managed agent are preserved across reinstall. The installer never grants a `permission: \"allow\"` (or any other) bypass on `agent.lore`. Ownership contract: opencode.json mcp.lore blocks are only overwritten when recognizably Lore-owned (legacy managed_by marker, or the same remote /v1/mcp URL with Authorization); a non-Lore-owned mcp.lore block fails closed with a typed conflict error, a backup of the existing file at the managed backup root, and a resolution message that names the conflicting block. The tui.json file is always Lore-owned.")
		fs.PrintDefaults()
	}
	if err := fs.Parse(args); err != nil {
		return 1
