...
func (a *App) runInstall(_ InteractiveActions, args []string) int {
	...
	fs.Usage = func() {
		...
		fmt.Fprintln(a.Stderr, "OpenCode writes bounded config-only Lore files into ~/.config/opencode: AGENTS.md, skills/*.md, and ~/.config/opencode/opencode.json with a Lore-owned top-level block. Optional lore-server-mcp writes a mcp block (plaintext bearer token in opencode.json). Commands and prompts are optional assets installable via --component opencode-sdd-assets; they are installed assets only, not runtime-wired or subagent claims. Commands/prompts are omitted unless explicitly selected. OpenCode non-goals remain explicit: no plugins, profiles, bootstrap/package-manager behavior, or native/runtime subagent wiring.")
		...
		fmt.Fprintln(a.Stderr, "OpenCode install does not write plugins, profiles, bootstrap/package-manager files, or native/runtime subagent wiring. When lore-server-mcp is selected, the bearer token is written in plaintext to ~/.config/opencode/opencode.json (mcp block). Commands/prompts are omitted unless explicitly selected via --component opencode-sdd-assets. They are installed assets only, not runtime-wired or subagent claims.")
