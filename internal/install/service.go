...
func supportedTarget(adapter HarnessAdapter) Target {
	...
	switch adapter.ID() {
	case TargetPi:
		target.ShortHelp = "Recommended default; Pi-native Lore setup."
		target.Details = "Recommended today; uses hosted Lore MCP via pi-mcp-adapter and no-auth Context7 MCP as default backends, with optional explicit pi-extensions (lore-footer.ts only) via --component pi-extensions. The deprecated lore-memory extension has been removed and is not available."
	case TargetAntigravity:
		target.ShortHelp = "Full Gemini prompt/profile projection."
		target.Details = "Full Antigravity projection with shared ~/.gemini/GEMINI.md prompt, variant-owned skills/manifest, managed Gemini lore agent profile, and global ~/.gemini/config/mcp_config.json direct MCP config for Lore plus no-auth Context7; no Pi overlays, daemon, or autostart behavior."
	case TargetOpenCode:
		target.ShortHelp = "Bounded config-only OpenCode projection."
		target.Details = "Bounded OpenCode projection: manages ~/.config/opencode/AGENTS.md, skills/<phase>/SKILL.md, a native opencode.json (with the documented $schema, default_agent=lore, native `agent` overlay wiring the primary `lore` orchestrator (`mode: \"primary\"`) plus the `lore-worker` repository worker and every SDD phase agent (each non-lore agent uses `mode: \"subagent\"`) to its SKILL.md via {file:...} references, optional native `agent.<name>.variant` preservation from existing opencode.json, and a native `skills` block — no top-level Lore-only `lore` metadata, no `permission: \"allow\"` bypass on `agent.lore`), and manifest. Lore MCP and Context7 MCP render by default as documented top-level mcp.lore and mcp.context7 remote entries without Lore-only marker fields; the installer fails closed with a typed conflict error and backup-path guidance when the existing mcp.lore or mcp.context7 block is foreign. tui.json uses the native OpenCode $schema and an empty singular `plugin` string array; Lore-managed plugin files are not copied to ~/.config/opencode/plugins/. Local Lore-owned runtime emulation/statusline plugins (background-agents.ts, lore-models.ts, model-variants.ts, and opencode-subagent-statusline.ts) are backup-deleted through manifest-scoped stale-file cleanup only when prior manifest ownership proves they are safe to remove. Explicit sdd-engram and logo plugins are never bundled or registered. Legacy installs that shipped the previous top-level `lore` block (in opencode.json) or the plural `plugins` array of objects (in tui.json) are silently repaired to the native shape on the next run. config-only projection: no profiles, bootstrap, Pi-like delegation emulation, model hot-edit plugin, or runtime subagents."
	case TargetCodex:
		target.ShortHelp = "Codex prompt, skills, MCP config, and manifest."
		target.Details = "Full Codex projection into ~/.codex with canonical AGENTS.md prompt, skills/*.md, remote Lore plus no-auth Context7 MCP config in config.toml, and manifest. Context7 receives no tool auto-approvals. No codex exec runner, daemon, Pi overlay, plugin, or bootstrap behavior is installed."
	default:
		target.ShortHelp = "Supported target."
		target.Details = "Supported target."
	}
	target.Description = target.Details
	return target
}
...
func roadmapTarget(id TargetID) Target {
	...
	case TargetClaudeCode:
		return Target{ID: id, Title: "Claude Code", Description: "Listed for roadmap visibility.", ShortHelp: "Roadmap target.", Details: "Listed for roadmap visibility.", Availability: "Coming soon"}
	default:
		return Target{ID: id, Title: string(id), Description: "Listed for roadmap visibility.", ShortHelp: "Roadmap target.", Details: "Listed for roadmap visibility.", Availability: "Coming soon"}
	}
}
...
func FormatTargetSelection(targets []Target) string {
	...
	for _, target := range targets {
		...
		fmt.Fprintf(&b, "- %s: %s (%s)\n", label, help, target.Availability)
	}
