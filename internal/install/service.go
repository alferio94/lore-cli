...
func supportedTarget(adapter HarnessAdapter) Target {
	...
	case TargetOpenCode:
		target.Description = "Bounded OpenCode projection: manages ~/.config/opencode/AGENTS.md, skills/<phase>/SKILL.md, a native opencode.json (with the documented $schema, default_agent=lore, native `agent` overlay wiring the primary `lore` orchestrator (`mode: \"primary\"`) plus the `lore-worker` repository worker and every SDD phase agent (each non-lore agent uses `mode: \"subagent\"`) to its SKILL.md via {file:...} references, optional `agent.<name>.variant` from a previous user-driven opencode.json hot-edit so user-chosen variants survive reinstall, and a native `skills` block — no top-level Lore-only `lore` metadata, no `permission: \"allow\"` bypass on `agent.lore`), and manifest. Lore MCP renders by default as a documented top-level mcp.lore remote entry without Lore-only marker fields; the installer fails closed with a typed conflict error and a backup when the existing mcp.lore block is foreign. tui.json uses the native OpenCode $schema and a singular `plugin` string array registering only the community opencode-subagent-statusline. Local plugin .ts files (background-agents.ts, lore-models.ts, opencode-subagent-statusline.ts) are copied to ~/.config/opencode/plugins/; only the community statusline is registered in tui.json. The `lore-models.ts` plugin keeps the provider/model/variant discovery cache behavior of the previous `model-variants.ts` asset and additionally exposes a safe opencode.json hot-edit entrypoint for in-OpenCode model/variant configuration. Explicit sdd-engram and logo plugins are never bundled or registered. Legacy installs that shipped the previous top-level `lore` block (in opencode.json) or the plural `plugins` array of objects (in tui.json) are silently repaired to the native shape on the next run. config-only projection: no profiles, bootstrap, or runtime subagents."
	case TargetCodex:
