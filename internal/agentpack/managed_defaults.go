...
func newSDDPhaseAsset(phase PhaseID, description, obligations string) AgentInstructionAsset {
	...
	phaseName := PhaseEnvelopeName(phase)
	mcpGuidance := bulletize(append(LoreMCPGuidance(), LoreArtifactPersistenceGuidance()...))
	...
	return AgentInstructionAsset{
		...
		InheritProjectContext: true,
		Body: PromptAsset{
			Template:  fmt.Sprintf("You execute the SDD %s phase.\nBefore work, MUST load and follow the resolved phase skill; resolve project-local before Lore-wide:\n- `%s`\n- `%s`\nObligations:\n%s\n- Decision: `needs_user_input`.\n\nLore MCP context and memory tool selection (canonical):\n%s\n\nReturn ONLY the compact SDD JSON envelope with keys %s, and set `phase` to `%s`. Final output status must be one of: `%s`. Do not use `running`, `next`, `executive_summary`, or `next_recommended`. This is the Pi Lore delegation adapter contract. %s\n%s\n", phaseName, skillRef.placeholder(), sharedPhaseCommon.placeholder(), obligations, mcpGuidance, EnvelopeFieldList(FinalEnvelopeFields), phaseName, FinalStatusList(), RuntimeOwnershipGuidance(), RepositoryMarkdownRuntimeBoundary()),
