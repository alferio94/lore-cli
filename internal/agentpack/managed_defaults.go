...
func newSDDPhaseAsset(phase PhaseID, description, obligations string) AgentInstructionAsset {
	...
	return AgentInstructionAsset{
		...
		InheritProjectContext: true,
		Body: PromptAsset{
			Template:  fmt.Sprintf("You execute the SDD %s phase.\n\nBefore substantial work, load and follow exactly:\n- `%s`\n- `%s`\n\nPhase obligations:\n%s\n- If a decision is required, stop with `needs_user_input`.\n\nLore memory tool selection (canonical):\n- Prefer MCP Lore Server tools (`lore_memory_*`) over deprecated harness-local memory extensions. The Pi-native `lore-memory.ts` extension was removed and is not available in any install path.\n- Use `lore_memory_search` for discovery. Search is filter-driven: pass `type`, `scope`, and `limit`; do not pass query text. The result includes compact `content_preview` and OMITS full `content`.\n- `lore_memory_search` accepts exactly one of `project_id` (UUID) or `project_key` per call. Prefer `project_key` when a stable key is known; only use `project_id` when no key is available.\n- To load the full body, call `lore_memory_get` with `project_id` (UUID) plus the memory `id`. `lore_memory_get` requires `project_id`; passing `project_key` is not a supported substitute.\n- Harness-local or harness-native fallback tools may have older schemas and MUST only be used when MCP Lore Server tools are unavailable. Do not mix the two surfaces in the same workflow.\n\nReturn ONLY the compact SDD JSON envelope with keys `status`, `phase`, `summary`, `artifacts`, `files`, `validations`, `risks`, `next_step`, `continuation`, `question`, `options`, `skill_resolution`, and set `phase` to `%s`. Final output status must be one of: `completed`, `needs_user_input`, `failed`. Do not use `running`, `next`, `executive_summary`, or `next_recommended`. This is the Pi Lore delegation adapter contract; Codex/Antigravity do not consume this exact JSON shape. Delegation is provided by the `lore-pi-runtime` package; the legacy `lore-delegation.ts` Pi extension is currently disabled.\n", phaseName, skillRef.placeholder(), sharedPhaseCommon.placeholder(), obligations, phaseName),
