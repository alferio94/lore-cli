...
func newModel(actions cli.InteractiveActions) model {
	...
	m := model{
		...
		items: []menuItem{
			...
			{key: "install", title: "Install", description: "Pi is recommended today; Codex writes managed remote MCP + skills, Antigravity is the prompt + skills MVP target, OpenCode writes a bounded config-only projection with the opencode-plugins bundle (background-agents, model-variants, opencode-subagent-statusline referenced from tui.json, sdd-engram/logo never bundled or registered) and a fail-closed mcp.lore ownership check (the installer only overwrites a mcp.lore block that already carries managed_by: lore-cli; foreign blocks fail closed with a backup), and Claude Code remains Coming soon."},
...
func (m model) renderInstallTargetSelection() string {
	...
	fmt.Fprintf(&b, "\nSelected target: %s. Use ↑/↓ to switch between supported targets and Enter to continue. Pi remains the default recommended path. Codex writes managed remote MCP + skills, Antigravity can write ~/.gemini/config/agents/lore.json and optionally write direct MCP config, and OpenCode writes a bounded config-only projection under ~/.config/opencode/ with optional MCP. OpenCode defaults include the opencode-plugins bundle (background-agents.ts and model-variants.ts copied to plugins/, plus the community opencode-subagent-statusline referenced from tui.json — the local plugin .ts files are NOT registered in tui.json) and never bundle the explicit sdd-engram or logo exclusions. The installer enforces a fail-closed mcp.lore ownership check: a non-Lore-owned mcp.lore block (missing the managed_by: lore-cli marker or owned by another tool) fails closed with a backup of the existing opencode.json and a clear resolution message.", selected.Title)
	return b.String()
}
